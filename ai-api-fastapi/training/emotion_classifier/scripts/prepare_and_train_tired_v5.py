# 파일: prepare_and_train_tired_v5.py
# 역할: TIRED 데이터 보강 후 6-class 감정 분류 모델 학습
# 실행: python scripts/prepare_and_train_tired_v5.py
...
ARTIFACTS_DIR = BASE / "artifacts" / "tired_v5"
...
if BOUNDARY_CSV.exists():
    ...
    boundary_rows = [{"text": str(r["text"]).strip(), "label": r["label"]} for _,
    r in boundary_df.iterrows()]
...
# SAD/ANGRY는 더 많은 샘플로 다양성 확보
CAP_PER_CLASS = {"HAPPY": 5000, "CALM": 5000, "ANXIOUS": 5000, "SAD": 10000, "ANGRY": 8000}
...
for label, cap in CAP_PER_CLASS.items():
...
train_df.to_csv(PROCESSED_DIR / "train_tired_v5.csv", index=False)
valid_df.to_csv(PROCESSED_DIR / "valid_tired_v5.csv", index=False)
...
# SAD/ANGRY에 1.5배 추가 부스트
BOOST = {"SAD": 1.5, "ANGRY": 1.5}
class_weights = torch.zeros(num_classes)
for label, idx in LABEL2ID.items():
    count = label_counts.get(label, 1)
    class_weights[idx] = total / (num_classes * count) * BOOST.get(label, 1.0)
...
eval_path = BASE / "artifacts" / "evaluation" / "valid_metrics_tired_v5.json"
